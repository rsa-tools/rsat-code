...
class SiteSetOnSameSeq(object):
	...
	def sort(self):
		...
		self.__sites_list = sorted(self.__sites_list,
								   key=lambda x: x.center())  #this function sorts by center's position
		self.__sorted = True
...
def read_features(number_of_matrix, autoparam, file_name, pval_threshold, lth_score_threshold, uth_score_threshold,
				  verbose):
	"""
...
def read_stdin(number_of_matrix, autoparam, stdin, format, pval_threshold, lth_score_threshold, uth_score_threshold,
			   verbose):
	"""
		...
		for line in tab_list:
			...
			if line[0] != ";" and line[0] != "#":
				...
				# Extraction of sites informations
					...
					sites_list.append(
						line)  # Add this list of table elements (site's informations) to the list of sites
...
def write_crer(crer, filehandle):
	...
	filehandle.write("%s	%s	%s	%s	%d	%d	%d	%.2g	%.2e	%.2g	%.2g	%.2g	%d	%d\n" % (
		crer[0], "CRER", crer[1], crer[2], crer[3], crer[4], crer[5], crer[6], crer[7], crer[8], crer[9], crer[10],
		crer[11], crer[12]))
...
if __name__ == '__main__':
	...
	parse = argparse.ArgumentParser(
		description = "\n".join((
			"Taking as input a set of genomic features (sites), detect cis-regulatory enriched regions (CRER), i.e. genomic intervals containing a higher number of sites than expected by chance.",
			...
			"(i) Predictions obtained by scanning sequences with position-specific scoring matrices (for example the output of matrix-scan.",
    		"(ii) Peaks from ChIP-seq experiments.",
    		"(iii) Annotated binding sites imported from a transcription factor database.",
    		"(iv) Any other data source that produces a set of genomic features.")),
        epilog="Marie Artufel*, Lucie Khamvongsa* and Jacques van Helden")

parse.add_argument('-i', action='store', dest='file', default=False, help="If not specified, input is read from STDIN ")
parse.add_argument('-in_format', action='store', dest='format', default='ft',
				   help="input_format. Default: ft (produced by RSAT matrix-scan and dna-pattern). Supported: ft, bed")
parse.add_argument('-o', action='store', dest='outfile', help="Output file in ft format")
parse.add_argument('-v', action='store', dest='verbose', default=1, type=int,
				   help="level of verbose. Messages are wrote on standard error. Supported: Integer = 1,2,3. By default : 1 = No message. Level 2 : moderately density of messages. Level 3 : High density")

parse.add_argument('-autoparam', action='store', dest='autoparam', default="off",
				   help="Extract some input parameters from the commented rows (starting with ';') of the input file. This option is only valid for files produced by matrix-scan with a verbosity of at least 1. ")
parse.add_argument('-s', action='store_true', dest='sort', default=None,
				   help="sort the list of sites. Very recommended. The sites are sorted by center position")
parse.add_argument('-return_limits', action='store_true', dest='return_limits', default=None,
				   help="return every limits of sequences. By default : no return any limits")
parse.add_argument('-return_limits_filtered', action='store_true', dest='return_limits_filtered', default=None,
				   help="return the limits filtered of the sequence. Only the sequence limits of CRERs. By default : no return any limits")
parse.add_argument('-lth_score', action='store', dest='lth_site_score', default=None, type=float,
				   help="minimal site score to be considered")
parse.add_argument('-uth_score', action='store', dest='uth_site_score', default=None, type=float,
				   help="maximal site score to be considered")
parse.add_argument('-uth_site_pval', action='store', dest='uth_site_pval', default=1e-4, type=float,
				   help='maximal p_value of sites to be considered. recommended to be the higher site p_value considered')
parse.add_argument('-number_of_matrix', action='store', dest='number_of_matrix', type=int, default=0,
				   help="number of matrix used for the discovery of transcription factor binding sites. Warning : Not considered if autoparam is on")  # Arguments for return CRERs
parse.add_argument('-lth_crer_size', action='store', dest='lth_crer_size', default=None, type=float,
				   help="minimal size of the enriched region (in bp). Default: minimal size = 30bp")
parse.add_argument('-uth_crer_size', action='store', dest='uth_crer_size', default=None, type=float,
				   help="maximal size of the enriched region (in bp). Default:  maximal site = 500bp")

parse.add_argument('-lth_crer_sites', action='store', dest='lth_crer_sites', default=None, type=float,
				   help="minimal number of sites covered by the enriched region. Default: minimal number of sites = 2")
parse.add_argument('-uth_crer_sites', action='store', dest='uth_crer_sites', default=None, type=float,
				   help="maximal number of sites covered by the enriched region. ")

parse.add_argument('-lth_crer_sites_distance', action='store', dest='lth_crer_sites_distance', default=None, type=float,
				   help="distance between successive sites to be considered. A minimal inter-site distance can be used to prevent overlap between redundant matrices. Default = minimal distance = 1")
parse.add_argument('-uth_crer_sites_distance', action='store', dest='uth_crer_sites_distance', default=None, type=float,
				   help="distance between successive sites to be considered. A maximal inter-site distance can be used to prevent merging distinct modules into a single one. Note: the maximal inter-site distance is one of the most influential parameters in cluster-buster. Default: maximal distance = 35")

parse.add_argument('-uth_crer_pval', action='store', dest='uth_crer_pval', default=None, type=float,
				   help="maximal binomial p-value. Default: 1e-4")

parse.add_argument('-uth_crer_eval', action='store', dest='uth_crer_eval', default=None, type=float,
				   help="maximal e-value. Default: 1e-4")

parse.add_argument('-lth_crer_sig', action='store', dest='lth_crer_significance', type=float, default=None,
				   help="minimal binomial significance. Default: 2")

parse.add_argument('-uth_overlap', action='store', dest='uth_overlap', default=None, type=int,
				   help="maximal overlap to define two distinct sites")

parse.add_argument('-nopval', action='store_true', dest='nopval', default=False, help="compute CRER without p value")
parse.add_argument('-pre_table', action='store_true', dest='pre_table', default=False,
				   help="compute a table where is all possible p_value. Useful where there is a huge number of sites to scan.")  # Arguments extraction
args = parse.parse_args()  # Variables for arguments extraction
...
nopval = args.nopval
pre_table = args.pre_table  #===========================================================================  # Parsing  #===========================================================================  # Do not accept '(,)' characters in output filename
if outfile:
	expression_reg = re.compile("[\(\) ]")
corresp = expression_reg.search(outfile)

if corresp:
	raise IOError("Forbidden character in the outfile's name")  # Verbose message
...
if file == False:
	...
	try:
		list_site, init_table, number_of_matrix, max_site_pvalue = read_stdin(number_of_matrix, autoparam, readline,
	...
	if format == "ft":  #test of the input file format
		...
		try:
			list_site, init_table, number_of_matrix, max_site_pvalue = read_features(number_of_matrix, autoparam, file,
																					 pval_threshold,
																					 lth_score_threshold,
																					 uth_score_threshold, verbose)
		...
		try:
			list_site, init_table, number_of_matrix = read_bed(number_of_matrix, file, lth_score_threshold,
															   uth_score_threshold, verbose)
...
filehandle.write(
	";	5	start = starting position of the crer, which corresponds to the start position of the leftmost site\n")
filehandle.write(
	";	6	end = end position of the crer, which corresponds to the end position of the rightmost site\n")
...
filehandle.write(
	";	14	crer_center_size = size taken in consideration for the p-value computation -> from center of the leftmost site to the center of the rightmost site (this size thus differs from the difference between crer_start and crer_end).\n")


# Write the columns
filehandle.write(
	"#seq_id	ft_type	ft_name	strand	start	end	hit_sum	crer_sig	crer_eval	crer_pval	hit_pval_product	weight_sum	crer_size	crer_center_size\n")
...
for SiteSetOnSameSeq in SiteSet_object.site_set_list():
		...
		if seq[0] == seq_id:
			...
			if return_limits:
				...
				if nopval:
					...
					filehandle.write(
						"%s	limit	START_END	D	%s	%s	.	0	0	0	0	0	0\n" % (
							seq[0], seq[1], seq[2]))
	...
	for i in range(0, len(list_site_one_seq) - 1):
		...
		for j in range(i + 1, len(list_site_one_seq)):
			...
			overlap = (min(list_site_one_seq[j].end(), list_site_one_seq[j - 1].end()) - max(
				list_site_one_seq[j].start(), list_site_one_seq[j - 1].start())) / minimal_site_size
							...
							if nb_sites >= exp_sites:
								...
								if not nopval:
									...
									if uth_pval_threshold and uth_eval_threshold:
										...
										crer_end_center = max(list_site_one_seq[j].center(),
															  list_site_one_seq[i].center())
										...
										data = "%s	%s	%s	%d	%d	%d	%.2g	%.2g	%.2g	%d	%d	%d\n" % (
											...
											0,
											crer_pval, pval_prod, weight_crer, crer_size, size)
								...
								if nopval:
									...
									crer = [list_site_one_seq[i].seq_id(), "crer", "DR", crer_start, crer_end, nb_sites,
											crer_sig, e_val, crer_pval, pval_prod, weight_crer, crer_size, size]
									...
									if return_limits_filtered:
										for seq in init_table:
											if crer[0] == seq[0]:
												filehandle.write(
													"%s	limit	START_END	D	%s	%s	.	0	0	0	0	0	0	0\n" % (
														seq[0], seq[1], seq[2]))
		...
		for ligne in read:
			...
			# E-value depend on number of expected crer
				...
				if lth_sig_threshold:
					...
					crer = [crer_tmp[0], crer_tmp[1], crer_tmp[2], int(crer_tmp[3]), int(crer_tmp[4]), int(crer_tmp[5]),
							crer_sig, e_val, float(crer_tmp[7]), float(crer_tmp[8]), int(crer_tmp[9]),
							int(crer_tmp[10]), int(crer_tmp[11])]
