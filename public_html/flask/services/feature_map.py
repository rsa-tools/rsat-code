...
class VariationInfo(Resource):
	...
	def _run(self, data):
		...
		fileupload_parameters = ['i','colors','seq']
		...
		boolean_var = ['symbol', 'legend', 'scalebar', 'no_name', 'scorethick', 'htmap', 'mono', 'aacolors', 'horiz', 'vertic']		
		for param in data:
		...
		command += ' -o ' + result_dir + '/output.' + data['format']  
		return utils.run_command(command, output_choice, tool, 'html', result_dir)
