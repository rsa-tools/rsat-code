...
#     open(STDERR,">","$outputdir/$file.log");
    ...
    $neworkfilepattern = "$basedir/networks/$neworkfilepattern/$network";
    my $networknodenames = "$neworkfilepattern"."-metab-node_names.tab";
    my $networkfile = "$neworkfilepattern"."-metab-network.tab";
...
# 	$file = $dirtoget.$files;
    ...
    return SOAP::Data->name('response' => {'organismslist' => $ret});
...
#     return $ret;
  ...
  ##########################################
  sub supported_networks_metab{
    my $dirtoget="$ENV{RSAT}/public_html/data/metabolic_networks/networks/";
    my @file_list =  &find_files($self,"$ENV{RSAT}/public_html/data/metabolic_networks/networks/","-network\\.tab\$");
    sort (@file_list);
    foreach(@file_list)
    {
      s/_network.tab$//g;
    ...
    my $ret = join ("\n", @file_list);
    print STDERR $ret;  
    return SOAP::Data->name('response' => {'networkslist' => $ret});;
  }
...
#   {
#      print $ENV{RSAT}."\n";
#     print &supported_organisms_metab()."\n";
#  print &supported_networks_metab()."\n";
# 
#   } 
