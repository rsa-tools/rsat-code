...
To run this tutorial on the command line, you need to have both \program{RNSC} and \program{MCL} installed on your computer. You can find the MCL source code on \textit{http://micans.org/mcl/} and \program{RNSC} on \textit{http://rsat.scmbb.ulb.ac.be/~rsat/rnsc/rnsc\_rewritten\_compiled32.zip}. 

\subsection{Protocol for the web server}

\subsubsection{Dataset download}
Go on the NeAT demo dataset web page (\textit{http://rsat.scmbb.ulb.ac.be/rsat/data/neat\_tuto\_data/}) and download the MIPS complexes (\file{mips\_complexes.tab})..
...
As a preliminary step, go on the NeAT demo dataset web page (\textit{http://rsat.scmbb.ulb.ac.be/rsat/data/neat\_tuto\_data/}) and download the MIPS complexes (\file{mips\_complexes\_names.tab}) and the Gavin interaction dataset (\file{gavin\_2006\_names.tab}).
