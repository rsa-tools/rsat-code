...
\section{Compiling C/C++ programs}

Some of the tools available in \RSAT (info-gibbs) are written in the
C/C++. The distribution only contains the sources of these tools,
because the binaries are operating system-dependent. The programs can
be compiled in a very easy way.
...
\begin{itemize}
\item[info-gibbs] a gibbs sampling algorithm based on optimization of
  the the information content of the motif.
\item[count-words] an efficient algorithm for counting word
...
\item[matrix-scan-quick] an efficient algorithm for scanning sequences
  with a position-specific scoring matrix. As its name indicates,
  \program{matrix-scan-quick} is \emph{much} faster than the Perl
  script \program{matrix-scan}, but presents reduced functionalities
  (only computes the weight, returns either a list of sites or the
...
The KWALKS\_ROOT variable in the RSAT config file should point to \$RSAT/contrib/kwalks/bin
...
The REA\_ROOT variable in the RSAT config file should point to \$RSAT/contrib/REA
