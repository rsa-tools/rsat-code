...
After having sent the signed license, you should have received an
email with the login and password to the \RSAT download site. 
...
with the command \program{tar}, which are part of the default unix
installation.
...
located in the \file{rsa-tools} directory:
\begin{itemize}
\item \file{RSAT\_config\_default.props} 
\item \file{RSAT\_config\_default.mk}
\end{itemize}

You must copy these two file to create site-specific configuration
files \file{RSAT\_config.props} and \file{RSAT\_config.mk}, which will
be further edited.
...
Then, edit the file \file{RSAT\_config.props} and specify the email of
the RSAT administrator (likely you).
...
\item \program{Util::Properties} is required to load property files,
  ...
  \RSAT server. Property filesa are also useful to write your own perl
  clients for the Web service interface to \RSAT (RSATWS).

\item \program{SOAP::WSDL}, \program{SOAP::Lite},
  \program{Module::Build::Compat} and \program{Util::Properties}
  (already mentioned above) are required for the Web services. \RSAT
  Web services is a convenient interface that permits to write Perl
  scripts to run \RSAT queries on a remote server. Some Perl scripts
  of the \RSAT stand-alone commands are using SOAP to connect remote
  servers (e.g. \program{supported-organisms},
...
cvs -d :pserver:cvsuser@cvs.sanger.ac.uk:/cvsroot/ensembl \
  checkout -r branch-ensembl-64 ensembl
## (you need to adapt the branch number, e.g. 64, as it updates).

## Download the current version of the EnsEMBL module for comparative genomics
cvs -d :pserver:cvsuser@cvs.sanger.ac.uk:/cvsroot/ensembl \
  checkout -r branch-ensembl-64 ensembl-compara
## (you need to adapt the branch number, e.g. 64, as it updates).
\end{lstlisting}
%%$ 
...
\begin{description}
\item[\program{vmatch}]: developed by Stefan Kurtz, is used used by
  the program \program{purge-sequences}, to mask redundant sequences
  that bias motif discovery statistics.

\item[\program{seqlogo}]: developed by Thomas D. Schneider, is used
  used by the program \program{convert-matrix} to generate
