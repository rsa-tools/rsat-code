...
The simplest way to install a Perl module is to run the \program{cpan}
installer via \program{perl}. For example, to install the module
\program{Postscript::Simple}, type the following.
...
\item[File::Spec, POSIX and Data::Dumper] re required for some
  functions of \program{matrix-scan}.
...
\item[DBD::mysql and DBI] those two libraries are required by the
  program \program{retrieve-ensembl-seq} in order to access the
  ENSEMBL database.
...
The standard version of the program \program{XYgraph} export figures
...
install the freeware software \program{gnuplot} (4.2 or later), which
