...
make -f makefiles/install_rsat.mk perl_modules_install
  ...
  \item \program{cpan} installation takes some time, and is interrupted
...
\begin{lstlisting}
supported-organisms-server -v 2 -server http://rsat.eu/ -o supported_on_rsat_eu.tab ; 
...
\begin{lstlisting}
supported-organisms-server -server http://rsat.fr/ -o supported_on_rsat_fr.tab ; 
...
The programs \program{vmatch} and \program{mkvtree} are used by the
\RSAT progam \program{purge-sequence}

\section{Activating a local Web server}

\include{ig_apache_config.tex}
...
\file{public\_html/web\_services/RSATWS.wsdl}
