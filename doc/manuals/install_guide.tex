...
\program{cvs}. For external users, there is no need to use
\program{cvs} since the code is distributed as a compressed archive on
a Web page.
...
\subsection{Obtaining the \RSAT package}
...
The \program{java} language is only necessary for the path finding and
pathway extraction tools of the Network Analysis Tools \program{NeAT}
suite. If you want to use these tools, you can check the correct
setting for the java-based pathway tools, by typing the following
command.
...
\item On \emph{Mac OSX} systems, the installation of the GD library is
  quite tricky. The best way I found is to follow the installation
  protocol of the
  libgd Web site \\
...
\item \program{JSON} is required for parsing the result of REST-based
  web services.
...
\item to access EnsEMBL versions above 47, you need port 5306 to be
  ...
  accepts this port.
\item On Mac OSX, there seems to be problem with the mysql bundle,
  which can be fixed by adding a soft link as follows\footnote{\url{http://www.blog.bridgeutopiaweb.com/post/how-to-fix-mysql-load-issues-on-mac-os-x/}}:
  \begin{lstlisting}
