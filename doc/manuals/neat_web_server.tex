...
In order for the server to work with \neat, you have to edit the file
\file{php.ini} which is the main configuration file of PHP. Depending
on your PHP installation, this file might be in different
directory. In computers running Suse 10.3, this file is in the
\url{/etc/php5/apache2/} directory. In this file, you must modify the
...
A part of \neat is using axis web services, JSP and Java servlet pages
and needs Tomcat or an equivalent servlet engine to run.  Tomcat can
be easily installed on SUSE with yast and is usually located in
\file{/usr/share/tomcat} after installation.
...
that Tomcat version 6 is more stable.  We will refer from now on to
the Tomcat root directory as \$CATALINA\_HOME.
