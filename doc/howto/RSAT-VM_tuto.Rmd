...
- Download the version of your coice (current version: rsat-vm-2016-03.ova). 
...
- In the import dialog box, click on the folder icon, and locate the appliance file (rsat-vm-2016-03.ova) that you downloaded from the RSAT Web site, and click on **Continue**. 
- Check the Appliance settings ([snapshot](images/import_appliance_settings.png)). 
- Activate the option *Reinitialize MAC address of all network cards*. 
...
In the panel showing the available virtual machines, right-click on the RSAT-VM (rsat-vm-2016-03), open the *Settings ...* dialog box. In the tab *Network*, select *Adapter 1*, check *Enable Network Adapter*, select *Attached to: Host-only Adapter* ([snapshot](images/vm_settings_network_host-only.png)).
...
- In the panel showing the available virtual machines, right-click on the RSAT-VM (rsat-vm-2016-03), open the *Settings ...* dialog box.

- In the tab *Network*, select <* class="option">Adapter 1*, check *Enable Network Adapter*, select *Attached to: Bridge adapter*. 
...
In the left panel of **VirtualBox**, select the virual machine (rsat-vm-2016-03), and click on the *Start* icon.
...
3. As SOAP/WSDL Web services (advanced use). 
...
You can log in with the following parameters:
...
1. At your first login, you will be prompted to change your password before anything else. It is important to choose a suitable non-trivial password, to prevent hackers from accessing your virtual machine. 
...
On Ubuntu 14.04 server version, keybord configuration can be modified with following command.

```
sudo dpkg-reconfigure console-data
```

For the desktop version, click on the *Settings* icon [![](images/icon_settings.png){width="40"}](images/icon_settings.png),
...
If you are familiar with the Unix command line environment, you can access your virtual machine from a terminal of the host machine, with the following command. 
