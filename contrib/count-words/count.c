...
//     } while (c != '>');
// 
//     string_buffer_set_char(buffer, '\0', i);
// 
//     if (c == '>')
//         return TRUE;
//     else
...
void count_occ(long *count_table, long *last_position, long *overlapping_occ, char *string, int oligo_length, int add_rc, int noov, int grouprc)
{
    if (noov)
...
void print_count_array(FILE *output_fp, long *count_array, long *overlapping_occ, int oligo_length, int add_rc)
{
    ...
    // header
    if (overlapping_occ)
        fprintf(output_fp, "#seq\tid\tobserved_freq\tocc\tovl_occ\n");
    else
...
void count_in_file(FILE *input_fp, FILE *output_fp, int oligo_length, int add_rc, int noov, int grouprc, int argc, char *argv[])
{
