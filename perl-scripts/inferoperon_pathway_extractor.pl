...
############################################################
#
# $Id: inferoperon_pathway_extractor.pl,v 1.7 2011/10/05 11:00:44 rsat Exp $
#
############################################################
...
inferoperon_pathway_extractor.pl -h [-i inputfile] [-o outputdirectory] [-v] -g graphfile -a gene2ec2reactions [-b gene2reaction]
...
#     $main::show;				# Open png image in gwenview
    ...
    ################################################################
    ## Parse infer operon file and launch pathway extractor
     print $outdir;
     my $oldoperonid;
     my @genelist;
     my $line;
...
# 	   print "$tempdata[6] eq $oldoperonid\n";
	   if ($oldoperonid && !($tempdata[6] eq $oldoperonid)){
	    $oldoperonid=~s/\s+/_/g;
	    my $tempfilename = $outdir.$oldoperonid."_original_seeds.txt";
	    chomp ($tempfilename);
	    open (MYFILE, '>'.$tempfilename);
	    print MYFILE join ("\n",@genelist). "\n";
	    close (MYFILE);
	    my $grfilecmd = "";
	    if ($main::grfile){ # if there is a file with direct link from gene to reaction
	      $grfilecmd = "-b $main::grfile";
	    }
	    my $pathway_infer_cmd = "pathway_extractor.pl $main::verbose -i $tempfilename -g $main::graphfile $grfilecmd -a $main::gprfile -t $main::outdir -o $main::outdir";
...
=cut
      ...
      &FatalError(join("\t", "Invalid inferoperon_pathway_extractor option", $arg));
