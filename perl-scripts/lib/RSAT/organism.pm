...
sub CalcNeighbourLimits {
    ...
    &RSAT::message::Info("Calculating neighbout limits") if ($main::verbose >= 1);
	...
	for my $g (0..$#genes) {
	    ...
	    $found = 0;
	    do {
		if (($ctg_lefts[$rn] < $right{$gene}) && ($ctg_rights[$rn] < $right{$gene})) {
		    $rn++;
	    ...
	    $gene->set_attribute("right_neighbour", $rn);
...
#				 ) if ($main::verbose >= 10);
		...
		$upstr_size = $neighb_right_size;
		
		## Downstream neighbour is on the left side
		$dn=$ln;
		$downstr_limit = $neighb_left_limit;
		$downstr_size = $neighb_left_size;
	    } else {
		## Upstream neighbour is on the left side
		$un = $ln;
		$upstr_limit = $neighb_left_limit;
		$upstr_size = $neighb_left_size;

		## Downstream neighbour is on the right side
		$dn = $rn;
		$downstr_limit = $neighb_right_limit;
		$downstr_size = $neighb_right_size;
	    }
	    $gene->set_attribute("upstr_neighbour", $genes[$un]);
	    $gene->set_attribute("upstr_limit", $upstr_limit);
	    $gene->set_attribute("upstr_size", $upstr_size);
	    $gene->set_attribute("downstr_neighbour", $genes[$dn]);
	    $gene->set_attribute("downstr_limit", $downstr_limit);
	    $gene->set_attribute("downstr_size", $downstr_size);

# 	    &RSAT::message::Debug ("neighbours", 
# 				   $g, $gene->get_attribute("id"), $gene->get_attribute("strand"), 
# 				   $left{$gene}, $right{$gene}, 
# 				   "left",  $ln, $ctg_lefts[$ln],$ctg_rights[$ln], 
# 				   $neighb_left_limit, $neighb_left_size,
# 				   "right", $rn, $ctg_lefts[$rn],$ctg_rights[$rn], 
# 				   $neighb_right_limit, $neighb_right_size,
# 				   "upstr", $upstr_limit, $upstr_size,
# 				   "down", $downstr_limit, $downstr_size)
# 		if ($main::verbose >= 10);
	}
