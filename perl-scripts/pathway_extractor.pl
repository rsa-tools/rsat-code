...
############################################################
#
# $Id: pathway_extractor.pl,v 1.15 2011/07/25 15:41:06 jvanheld Exp $
#
############################################################
...
This script uses the Pathwayinference program developped by Karoline
Faust Pathwayinference connects multipe genes node sets in a given
metabolic graph and returns the resulting pathway.

Pathwayinference wraps a number of algorithms to infer pathways: one
based on a k shortest paths algorithm (REA), one based on kWalks and a
hybrid approach combining both. In addition, two Steiner tree
algorithms are available (Takahashi-Matsuyama and Klein-Ravi), each of
them alone or in combination with kWalks.  Pathwayinference can also
be run in batch mode using seed node files
...
pathway_extractor -h -hp [-i inputfile] [-o outputdirectory] [-v verbosity] -g graphfile -a gene2ec2reactions [-b gene2reaction] [-d unique descriptor] [-t tempdi] [-show] 

=head1 INPUT FORMAT

=head2  1) Input file containing a list of gene ids: (Warning at least 2 gene ids must be present in -a -b and -g files)
    NP_414739
    NP_414740
    NP_414741
    NP_416617
