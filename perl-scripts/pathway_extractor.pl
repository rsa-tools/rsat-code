...
############################################################
#
# $Id: pathway_extractor.pl,v 1.10 2011/06/22 15:34:26 rsat Exp $
#
############################################################
...
pathway_extractor -h -hp [-i inputfile] [-o outputdirectory] [-v verbosity] -g graphfile -a gene2ec2reactions [-b gene2reaction] [-d unique descriptor] [-show] 
...
if ($seednum > 1) {
  ...
  #   chomp($tempfilename);
    ...
    my $pathway_infer_cmd = "java -Xmx1000M graphtools.algorithms.Pathwayinference -i $seed_converter_filename -m 5 -C -f flat -n -E $outdir -p $outdir -b -d -g $graphfile -y con -v -o $predicted_pathway_filename";
