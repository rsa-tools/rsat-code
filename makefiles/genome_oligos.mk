...
## and in different sequence types (coding, inergenic, ...).
...
## Calculate sequence length distributions
...
seq_len_distrib:
	...
	@${MAKE} one_seq_len SEQ_TYPE=genome
	@for type in intergenic_segments gene_segments ORF_sequences intergenic_segments_tandem intergenic_segments_convergent intergenic_segments_divergent; do \
		${MAKE} one_seq_len SEQ_TYPE=$${type} ; \
		${MAKE} one_seq_len_histo SEQ_TYPE=$${type} ; \
	done
...
SEQ_FILE=${SEQ_DIR}/${SEQ_PREFIX}.wc
...
#	for str in '-2str' '-1str' ; do 
	for str in '-1str' ; do \
		${MAKE} one_seq_one_markov_one_str STR=$${str} ; \
	done

## Iterate over selected oligo lengths and markov orders
one_seq_all_markov:
	@${MAKE} one_seq_one_markov OL=1 MKV=0
	@${MAKE} one_seq_one_markov OL=2 MKV=0
	@${MAKE} one_seq_one_markov OL=3 MKV=1
	@${MAKE} one_seq_one_markov OL=4 MKV=2
	@${MAKE} one_seq_one_markov OL=5 MKV=3

## all markov models for hexamers (just because I like them)
	@${MAKE} one_seq_one_markov OL=6 MKV=0
	@${MAKE} one_seq_one_markov OL=6 MKV=1
	@${MAKE} one_seq_one_markov OL=6 MKV=2
	@${MAKE} one_seq_one_markov OL=6 MKV=3
	@${MAKE} one_seq_one_markov OL=6 MKV=4

	@${MAKE} one_seq_one_markov OL=7 MKV=5
	@${MAKE} one_seq_one_markov OL=8 MKV=6
	@${MAKE} one_seq_one_markov OL=9 MKV=7
	@${MAKE} one_seq_one_markov OL=10 MKV=8


## Iterate over all sequence types
all_seq_all_markov:
	@for type in genome intergenic_segments ORF_sequences; do \
		${MAKE} one_seq_all_markov SEQ_TYPE=$${type}; \
	done
